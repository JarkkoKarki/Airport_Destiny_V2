...
function gameLoop(data) {
  ...
  function handleFlightAction(flyTurn) {
    ...
    marker.bindPopup(`${flyTurn + 1}. Airport: ${data[flyTurn]['name']}`).
      openPopup();
    ...
    document.querySelector('#lento1').
      addEventListener('click', function(event) {
        ...
        if (player.money >= planes[0].cost) {
          ...
          document.querySelector('#player-modal1').
            classList.
            add('hide');
          if (vuoro === 10) {
            alert('voitit Pelin');
  ...
  document.querySelector('.valinta1').
    addEventListener('click', EmissionAction);
  // Add event listener for the player's action button
  document.querySelector('.valinta').
...
async function initializeMap() {
  ...
  for (let i = 0; i < maxAirports; i++) {
    ...
    marker = L.marker([airport.latitude_deg, airport.longitude_deg]).
      addTo(map);
